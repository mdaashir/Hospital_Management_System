...
const AdminDashboard = () => {
	...
	const departments = {
		neurology: {
			name: 'Neurology',
			icon: <FaBrain className='mr-2 text-blue-500' />,
		},
		pediatrician: {
			name: 'Pediatrician',
			icon: <FaChild className='mr-2 text-blue-500' />,
		},
		gynecology: {
			name: 'Gynecology',
			icon: <FaFemale className='mr-2 text-blue-500' />,
		},
		cardiology: {
			name: 'Cardiology',
			icon: <FaHeartbeat className='mr-2 text-blue-500' />,
		},
		physiotherapy: {
			name: 'Physiotherapy',
			icon: <FaPersonWalking className='mr-2 text-blue-500' />,
		},
		general_physician: {
			name: 'General Physician',
			icon: <FaStethoscope className='mr-2 text-blue-500' />,
		},
		dentist: {
			name: 'Dentist',
			icon: <FaTooth className='mr-2 text-blue-500' />,
		},
		dermatology: {
			name: 'Dermatology',
			icon: <LiaBandAidSolid className='mr-2 text-blue-500' />,
		},
		gastrology: {
			name: 'Gastrology',
			icon: <GiStomach className='mr-2 text-blue-500' />,
		},
	};
	...
	return (
		<div className='min-h-screen bg-gradient-to-r from-blue-200 to-blue-400 flex flex-col lg:flex-row'>
			<div className='bg-white p-8 w-full lg:w-1/5'>
				<div className='mb-8 text-2xl font-bold text-center lg:text-left'>
					Admin Dashboard
				</div>
				<div className='mb-6'>
					...
					<ul>
						<li
							...
							onClick={() => handleOptionClick('viewPatients')}>
						...
						<li
							className='cursor-pointer flex items-center mb-2'
							onClick={() => handleOptionClick('modifyPatient')}>
				...
				<div className='mb-6'>
					...
					<ul>
						<li
							className='cursor-pointer flex items-center mb-2'
							onClick={() => handleOptionClick('viewDoctors')}>
							<FaUserMd className='mr-2 text-blue-500' />
							View All Doctors
						</li>
						<li
							className='cursor-pointer flex items-center mb-2'
							onClick={() => handleOptionClick('addDoctor')}>
							<FaPlus className='mr-2 text-blue-500' />
							Add Doctor
						</li>
						<li
							className='cursor-pointer flex items-center mb-2'
							onClick={() => handleOptionClick('modifyDoctor')}>
							<FaEdit className='mr-2 text-blue-500' />
							Modify Doctor
						</li>
						<li
							className='cursor-pointer flex items-center mb-2'
							onClick={() => handleOptionClick('removeDoctor')}>
							<TiDelete className='mr-2 text-blue-500' />
							Remove Doctor
						</li>
						<li
							className='cursor-pointer flex items-center mb-2'
							onClick={() => handleOptionClick('ViewAppointments')}>
							<SlCalender className='mr-2 text-blue-500' />
							View Appointments
						</li>
					</ul>
					...
					<div className='bg-white p-8 rounded-lg shadow-md'>
						...
						{departments[selectedOption] && (
							<DepartmentDoctorsView
								department={departments[selectedOption].name}
							/>
						)}
					</div>
				)}
